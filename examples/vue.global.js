  ...
  */
  ...
  function baseCreateRenderer(options, createHydrationFns) {
      ...
      const patch = (n1, n2, container, anchor = null, parentComponent = null, parentSuspense = null, isSVG = false, slotScopeIds = null, optimized = false) => {
          ...
          /**
           * 一个特殊的标志，表明差分算法应该退出优化模式。
           * 例如，renderSlot()在遇到非编译器生成的槽
           * (例如，手动编写的渲染函数，应该总是完全不同的)
           * 或手动创建的cloneVNodes时创建的块片段
           */
          ...
          switch (type) {
              ...
              default:
                  if (shapeFlag & 1 /* ELEMENT */) {
                      ...
                      /* 假设shapeFlag =4
                      * ShapeFlags.COMPONENT = 6
                      * 0000  0100    shapeFlag
                      * 0000  0110    ShapeFlags.COMPONENT
                      * 通过位运算，只有第 3 位的值为全为 true，故返回 true，其他位均返回 false。
      ...
      const patchKeyedChildren = (c1, c2, container, parentAnchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
          ...
          let e1 = c1.length - 1; // prev ending index /* 旧 vnode 最后一个节点的索引  */
          ...
          // 3. common sequence + mount(相同顺序+挂载)
          // (a b)
          // (a b) c
          // i = 2, e1 = 1, e2 = 2
          // (a b)
          ...
          // 4. common sequence + unmount (相同顺序+卸载)
          // (a b) c
          // (a b)
          // i = 2, e1 = 2, e2 = 1
          // a (b c)
          ...
          // i = 2, e1 = 4, e2 = 5
              ...
              // 5.2 loop through old children left to be patched and try to patch（遍历剩下的旧节点，找到胃patc的节点）
              // matching nodes & remove nodes that are no longer present （匹配节点，移除不再存在的节点）
              let j;
              let patched = 0; /* 记录在第 5 步patched新节点过的数量*/
              const toBePatched = e2 - s2 + 1; /* 没有经过 path 新的节点的数量 */
              let moved = false; /* 证明是否发生过移动 */
              // used to track whether any node has moved（用于跟踪是否有节点移动）
              ...
              for (i = s1; i <= e1; i++) {
                  ...
                  if (patched >= toBePatched) {
                      /* 5.2.1patched新节点过的数量 大于没有经过 path 新的节点的数量  */
                      /* 如果所有的新节点已经ptach 过了 只能移除旧节点剩下的了 */
                  ...
                  if (prevChild.key != null) {
                      /*5.2.2 如果,老节点的key存在 ，通过key找到对应的index */
                      ...
                      for (j = s2; j <= e2; j++) {
                          if (newIndexToOldIndexMap[j - s2] === 0 &&
                              ...
                              /* 5.2.4 newIndexToOldIndexMap[j - s2] === 0 新节点没有被patch */
                              /* 5.2.4.1 newIndexToOldIndexMap[j - s2] === 0 新节点没有被patch */
                              newIndex = j;
                              break;
                          }
                      }
                  ...
                  if (newIndex === undefined) {
                      /* 如果没有找到与旧节点对应的新节点，删除当前节点，卸载所有的节点 */
                      unmount(prevChild, parentComponent, parentSuspense, true);
                  }
                  else {
                      /* 把旧节点的索引，记录在存放新节点的数组中 */
                      // 新节点在旧序列的位置， newIndexToOldIndexMap = [0,0,0,0] // 0的作用是代表新增的节点，也就是说在就旧序列会找不到
              ...
              // 5.3 move and mount
              // generate longest stable subsequence only when nodes have moved（只在节点移动时生成最长稳定子序列）
              // longest stable subsequence（最长稳定子序列）
              /* 移动老节点创建新节点*/
              /* 根据最长稳定序列移动相对应的节点 */
              ...
              // looping backwards so that we can use last patched node as anchor（向后循环，以便我们可以使用最后一个修补过的节点作为锚）
              for (i = toBePatched - 1; i >= 0; i--) {
                  const nextIndex = s2 + i;
                  const nextChild = c2[nextIndex];
                  const anchor = nextIndex + 1 < l2 ? c2[nextIndex + 1].el : parentAnchor;
                  if (newIndexToOldIndexMap[i] === 0) {
                      /* 没有老的节点与新的节点对应，则创建一个新的vnode，因为我们初始 都为0 */
                      // mount new
                      patch(null, nextChild, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized);
                  }
                  else if (moved) {
                      // move if:
                      // There is no stable subsequence (e.g. a reverse) （没有稳定的子序列）
                      // OR current node is not among the stable sequence （当前节点不在稳定序列中）
  ...
  function getSequence(arr) {
      const p = arr.slice(); //  保存原始数据 slice 数组深拷贝方法
      ...
      for (i = 0; i < len; i++) {
          ...
          if (arrI !== 0) {
              ...
              while (u < v) {
                  // 果arrI <= arr[j] 通过二分查找，将i插入到result对应位置；u和v相等时循环停止
                  // 等同于 Math.floor((u+V)/2)
                  c = ((u + v) / 2) | 0; //  二分查找
      ...
      while (u-- > 0) {
          ...
          // 然后通过回溯赋值的方式解决了贪心 + 二分查找替换方式可能造成的值不正确的问题
  ...
  /* Demo */
  // let  numArr= [3, 1, 4, 2, 8, 9]
  // getSequence( numArr )
  // [1,3,4,5]  得到的 是 numArr 最长递增的索引，
  // 最终的值 应该是 [numArr[1],numArr[3],numArr[4],numArr[]5]
  // 也就是 [1,2,8,9]
  ...
  const FRAGMENT = Symbol( `Fragment` ); /* 碎片概念 */
